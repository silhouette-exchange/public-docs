...
Schema: [CreateMakerQuoteRequest](/docs/api/spec/schemas#createmakerquoterequest)
...
|--------|-------------|--------|
| `202` | Quote accepted; stored as `SUBMITTED`. | [CreateMakerQuoteResponse](/docs/api/spec/schemas#createmakerquoteresponse) |
...
| `403` | Credential is not maker-scoped. | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
...
| `status` | query | No | array of [QuoteStatus](/docs/api/spec/schemas#quotestatus) | Filter by one or more quote statuses. Repeat the key for multiple, e.g. `?status=SELECTED&status=SETTLED`. |
...
| Status | Description | Schema |
|--------|-------------|--------|
| `200` | The maker's quotes (`SELECTED` ones carry their permit). | [MakerQuotesPage](/docs/api/spec/schemas#makerquotespage) |
| `400` | Invalid request: the body or query parameters failed validation. The error `code` is `INVALID_REQUEST`; `details.errors` carries the field-level reasons. | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
| `401` | Unauthorized: missing or expired credentials, or an unknown access key. | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
| `403` | Credential is not maker-scoped. | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
| `500` | Internal server error. | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
...
| Status | Description | Schema |
|--------|-------------|--------|
| `200` | Quote cancelled; dropped from the auction. | [MakerQuote](/docs/api/spec/schemas#makerquote) |
| `401` | Unauthorized: missing or expired credentials, or an unknown access key. | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
| `403` | Quote belongs to another maker, or the credential is not maker-scoped. | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
| `404` | Not found. | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
| `409` | Quote is no longer open to cancel (already selected, settled, expired, or cancelled). | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
| `500` | Internal server error. | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
...
| Status | Description | Schema |
|--------|-------------|--------|
| `200` | Open RFQs the maker may quote. | [OpenRfqsPage](/docs/api/spec/schemas#openrfqspage) |
| `400` | Invalid request: the body or query parameters failed validation. The error `code` is `INVALID_REQUEST`; `details.errors` carries the field-level reasons. | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
| `401` | Unauthorized: missing or expired credentials, or an unknown access key. | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
| `403` | Credential is not maker-scoped. | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
| `500` | Internal server error. | [ErrorResponse](/docs/api/spec/schemas#errorresponse) |
